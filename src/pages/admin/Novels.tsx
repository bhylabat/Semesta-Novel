...
import { fetchNovels, fetchGenres, adminCreateNovel, adminUpdateNovel, adminDeleteNovel } from '@/lib/services';
    ...
    try {
      ...
      if (editing) {
        ...
        if (form.genreIds.length > 0) {
          await supabase.from('novel_genres').delete().eq('novel_id', editing.id);
          await supabase.from('novel_genres').insert(form.genreIds.map((gid) => ({ novel_id: editing.id, genre_id: gid })));
        }
      } else {
        const created = await adminCreateNovel(novelData);
        if (created && form.genreIds.length > 0) {
          await supabase.from('novel_genres').insert(form.genreIds.map((gid) => ({ novel_id: created.id, genre_id: gid })));
        }
      }
      setShowForm(false);
      loadData();
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-white">Manajemen Novel</h1>
      ...
      <div className="relative mb-4">
        ...
        <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cari novel..." className="input pl-9 w-full" />
      </div>
      ...
      {loading ? (
        ...
        <div className="card divide-y divide-white/5">
          {filtered.map((novel) => (
            <div key={novel.id} className="flex items-center gap-3 p-4">
              <div className="flex-1 min-w-0">
                <h3 className="text-sm font-medium text-white truncate">{novel.title}</h3>
                <p className="text-xs text-muted">{novel.author} · {formatViews(novel.views)} views · {formatDate(novel.created_at)}</p>
              </div>
              ...
              <button onClick={() => openEdit(novel)} className="btn-ghost p-2"><Edit className="h-4 w-4" /></button>
              <button onClick={() => setConfirmDelete(novel.id)} className="btn-ghost p-2 text-red-400"><Trash2 className="h-4 w-4" /></button>
            </div>
          ))}
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={() => setShowForm(false)}>
          <div className="card p-6 max-w-lg w-full max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-white">{editing ? 'Edit Novel' : 'Tambah Novel'}</h2>
              <button onClick={() => setShowForm(false)} className="btn-ghost p-2"><X className="h-5 w-5" /></button>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="text-sm text-white mb-1 block">Judul</label>
                <input type="text" value={form.title} onChange={(e) => { setForm({ ...form, title: e.target.value, slug: slugify(e.target.value) }); }} className="input w-full" required />
              </div>
              <div>
              ...
              <div>
                ...
                <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className="input w-full min-h-[100px]" required />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-sm text-white mb-1 block">Status</label>
                  <select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value as NovelStatus })} className="input w-full">
                ...
                <div>
                  ...
                  <input type="number" step="0.1" min="0" max="10" value={form.rating} onChange={(e) => setForm({ ...form, rating: parseFloat(e.target.value) })} className="input w-full" />
                </div>
              </div>
              <div>
                <label className="text-sm text-white mb-1 block">Genre</label>
                <div className="flex flex-wrap gap-2">
                  {genres.map((g) => (
                    <button key={g.id} type="button" onClick={() => {
                      const ids = form.genreIds.includes(g.id) ? form.genreIds.filter((id) => id !== g.id) : [...form.genreIds, g.id];
                      setForm({ ...form, genreIds: ids });
                    }} className={`badge ${form.genreIds.includes(g.id) ? 'bg-primary text-white' : 'bg-white/5 text-muted'}`}>
                      {g.name}
                    </button>
                  ))}
                </div>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={() => setConfirmDelete(null)}>
          <div className="card p-6 max-w-sm w-full" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-white mb-2">Hapus novel ini?</h3>
            <p className="text-sm text-muted mb-6">Semua bab terkait juga akan dihapus. Tindakan ini tidak dapat dibatalkan.</p>
