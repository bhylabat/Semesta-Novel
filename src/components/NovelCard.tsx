...
export default function NovelCard({ novel, rank, showLatestChapter }: NovelCardProps) {
  return (
    <Link to={`/novel/${novel.slug}`} className="group block">
      <div className="card card-hover overflow-hidden">
        ...
        <div className="p-3">
          ...
          <p className="mt-1 text-xs text-muted truncate">{novel.author}</p>
          ...
          {showLatestChapter && novel.latest_chapter && (
            <div className="mt-2 flex items-center justify-between border-t border-white/5 pt-2">
              <span className="text-xs text-muted truncate">Bab {novel.latest_chapter.chapter_number}</span>
              <span className="text-xs text-muted/70">{formatDate(novel.latest_chapter.created_at)}</span>
            </div>
          )}
        </div>
      </div>
